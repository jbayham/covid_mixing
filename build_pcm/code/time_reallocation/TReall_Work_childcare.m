...
chglist(rmidx)=[];
%Note: this next elimination must come after the first.  The .25 is
%conditional on only parents with childcare obligations
prop2=sum(kdat.co_sibs(uidx)==1 & ismember(kdat.occ(uidx),[10,11]))/sum(kdat.co_sibs(uidx)==1); %.25300625; %fraction of children who's parents work in healthcare and have childcare obligations
[toreall2,rmidx2]=datasample(chglist,round(prop2*numel(chglist)),'Replace',false);
chglist(rmidx2)=[];
...
save(sprintf('%s%s',tupath,'MixingDataset_HH_work_childcare.mat'),'contact','cut','labels','loclabels','-v7.3')
