...
save(sprintf('%s%s',tupath,'MixingDataset_HH_work.mat'),'contact','cut','labels','loclabels','-v7.3')
...
check1=reall_frac;
idx=reall_frac>.01;
bar(reall_frac(idx)*100)
xlim([0.5 size(check1(idx),1)+1])
%ylim([0 13])
tick_loc=(1:size(check1(idx),1));
set(gca,'XTick',[],'FontSize',10,'Position',[.1 .35 .88 .63])
set(gcf,'Position',[100 400 580 350]);
text(tick_loc,ones(length(tick_loc),1)-1.5,loclabels(idx),'Rotation',-40,'FontSize',8)
box('off')
ylabel('Percent of Parent Workplace')
end
