...
icontact=sortrows(icontact,[1 2]);

% 3) Need to find parents of school-aged kids who are unlikely to have
...
atwork=(icontact.saktotal>0 & icontact.act1==5 & icontact.act2==1 & icontact.where~=1 & icontact.actdur>0 & icontact.weekend==0 & icontact.holiday==0); %people at school
toreplace=icontact(atschool | atwork,{'start','stop','actdur','tucaseid','actnum','where'}); %id for teens who did report going to school set of target activities that will be modified 

donor=icontact(~(atschool | atwork),{'tucaseid','actnum','where','actdur'}); %nonschool activities  creating the donor subset
donor=donor(ismember(donor.tucaseid,unique(toreplace.tucaseid)),:); %limiting donor subset to only those who spent time at school during their diary day
...
save(sprintf('%s%s',tupath,'MixingDataset_HH_pubhome.mat'),'contact','cut','labels','loclabels','-v7.3')
...
check2=time_dist(contact(contact.person==1,:),locs);
idx=[2:12 21 23 24];
